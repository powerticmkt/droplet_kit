module DropletKit
  class ProjectResource < ResourceKit::Resource
    ...
    resources do
      ...
      action :find, 'GET /v2/projects/:uuid' do
        handler(200) { |response| ProjectMapping.extract_single(response.body, :read) }
      end
      ...
      action :update, 'PUT /v2/projects/:uuid' do
      ...
      action :delete, 'DELETE /v2/projects/:uuid' do
        handler(204) { |_| true }
        handler(422) { |response| ErrorMapping.fail_with(FailedDelete, response.body) }
      end

      action :list_resources, 'GET /v2/projects/:uuid/resources' do
        handler(200) { |response| ProjectAssignmentMapping.extract_collection(response.body, :read) }
      end

      action :assign_resources, 'POST /v2/projects/:uuid/resources' do
        verb :post
        body do |resources|
